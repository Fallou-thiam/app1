...
SECTIONS = [
    ...
    "Documentation"  # Nouvelle section ajoutée
]
...
Bienvenue sur le **Tableau de Bord Financier**, une application interactive conçue pour vous aider à analyser vos finances, explorer des concepts financiers et générer des rapports personnalisés au format PDF.
...
3. **Choix de la section préférée** :
   ...
   - Sélectionnez une section dans la liste déroulante pour commencer.
...
- **Barre latérale** :
  ...
  - Une astuce personnalisée apparaît selon votre âge (ex. : "Investissez tôt !" pour < 30 ans).
...
- **Comment l’utiliser** :
  ...
  3. **Exporter en PDF** :
     ...
     - Rendez-vous dans "Rapport Personnalisé", cochez "Analyse de Portefeuille", et générez le rapport.
...
- **Comment l’utiliser** :
  1. **Sélectionner les sections** :
     - Cochez les options que vous voulez inclure (ex. : "Analyse de Portefeuille").
     - Remplissez les champs demandés pour chaque option.
  2. **Générer le rapport** :
     - Cliquez sur "Générer le Rapport".
     - Téléchargez le fichier PDF via "Télécharger le Rapport PDF".
  3. **Notes importantes** :
     - Pour inclure l’**Analyse de Portefeuille**, configurez-la d’abord dans sa section (3.3).
     - Les graphiques dans le PDF sont en **noir et blanc**. Une future mise à jour pourrait permettre une exportation en couleur.

---

## 4. Conseils Pratiques
- **Entrées valides** : Utilisez des virgules pour séparer les actifs ou montants (ex. : "AAPL, MSFT" ou "1000, 2000").
- **Chargement** : Attendez que le spinner disparaisse pour les données externes.
- **Erreurs** : Vérifiez vos saisies si un message d’erreur apparaît.

---

## 5. Limitations Actuelles
- **Graphiques en PDF** : Exportés en noir et blanc (couleur à venir).
- **Données boursières** : Si un symbole est invalide, un avertissement s’affiche.
- **Actualités** : Limitées à 60 articles par actif ou recherche, en français.

---

## 6. Besoin d’Aide ?
- Lisez les messages d’erreur ou d’avertissement pour corriger vos saisies.
- Cette version est éducative et analytique, sans support direct.

Merci d’utiliser le **Tableau de Bord Financier** !
"""
...
if not st.session_state.authenticated:
    ...
    # Section "Simulation Monte Carlo"
        ...
        st.write(f"- **Pire scénario** : {worst_case:.2f} €")
        st.write(f"- **Scénario moyen** : {average_case:.2f} €")
        st.write(f"- **Meilleur scénario** : {best_case:.2f} €")
    ...
    # Section "Rapport Personnalisé"
        ...
        if inclure_portefeuille:
            ...
            if all_valid and len(actifs_list) == len(montants_list):
                ...
                graphs["Matrice de Corrélation"] = correlation_matrix
    ...
    # Section "Documentation"
        ...
        st.title(f"Documentation - {prenom}")
